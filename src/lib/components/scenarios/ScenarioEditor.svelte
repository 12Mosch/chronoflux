<script lang="ts">
  ...
  // Define a partial type for the scenario prop since it might be incomplete or coming from a different source
  interface Scenario {
    _id?: Id<"scenarios">;
    name: string;
    description: string;
  ...
  let initialWorldStateJSON = scenario ? JSON.stringify(scenario.initialWorldState, null, 2) : JSON.stringify({
    nations: [],
    relationships: [],
    globalEvents: []
  }, null, 2);
...
<Card class="w-full max-w-2xl mx-auto mt-8">
  ...
  <CardContent class="space-y-4">
    ...
    <div class="space-y-2">
      ...
      <Textarea id="description" bind:value={description} placeholder="Brief description of the scenario" />
    </div>
    ...
    <div class="space-y-2">
      ...
      <Textarea id="aiContext" bind:value={aiContext} placeholder="Historical context for the AI..." class="h-24" />
    </div>

    <div class="space-y-2">
      <label for="worldState" class="text-sm font-medium">Initial World State (JSON)</label>
      <Textarea id="worldState" bind:value={initialWorldStateJSON} class="font-mono text-xs h-64" />
      <p class="text-xs text-muted-foreground">Edit the JSON directly to configure nations and relationships.</p>
    </div>
