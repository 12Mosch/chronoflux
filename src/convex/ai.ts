/**
 * Convex AI Integration for ChronoFlux
 * Handles AI-powered turn processing using Ollama/Qwen3
 */
...
/**
 * Build Action Interpretation prompt based on PROJECT_PLAN.md
 */
...
/**
 * Call Ollama API from server-side
 */
...
/**
 * Process turn with AI - Main action
 */
export const processTurnWithAI = action({
	args: {
		gameId: v.id('games'),
	...
	handler: async (ctx, args) => {
		...
		const scenario = await ctx.runQuery(internal.scenarios.getScenarioById, {
		...
		const playerNation = game.playerNationId
			? await ctx.runQuery(internal.nations.getNationById, { nationId: game.playerNationId })
			: null;
		if (!playerNation) throw new Error('Player nation not found');

		// Get all nations for this game
		const allNations = await ctx.runQuery(internal.nations.getNationsForGame, {
			gameId: args.gameId
		});

		// Get relationships
		const relationships = await ctx.runQuery(internal.relationships.getRelationshipsForNation, {
			gameId: args.gameId,
			nationId: game.playerNationId!
		});

		// Get recent events (last 3 turns)
		const recentTurns = await ctx.runQuery(api.turns.getTurnsForGame, { gameId: args.gameId });
		const recentEvents = recentTurns
			.slice(0, 3)
			.flatMap((turn: Doc<'turns'>) => turn.aiResponse.events.map((e) => e.title));

		// Calculate current year
		const currentYear = scenario.startYear + game.currentTurn - 1;

		// Build world state for prompt
		const worldState = {
			playerResources: playerNation.resources,
			relationships: relationships.map((rel: Doc<'relationships'>) => {
				const otherNationId = rel.nation1Id === game.playerNationId ? rel.nation2Id : rel.nation1Id;
				const otherNation = allNations.find((n: Doc<'nations'>) => n._id === otherNationId);
				return {
		...
		try {
			...
			actionResponse = {
				...
				nation_reactions: {},
				resource_changes: {},
				narrative: `${playerNation.name} attempts to ${args.playerAction}. The consequences are still unfolding.`
			};
		}
		...
		try {
			...
			events = [
				{
					type: 'other',
					title: `${playerNation.name}'s Action`,
					description: actionResponse.narrative,
					affected_nations: [playerNation.name],
